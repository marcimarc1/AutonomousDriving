...
class morphingcommonroad:
    ...
    @property
    def generating_driving_corridor(self):
...
def constraint_interpolation(vehpos, pointsleft, pointsright):  # use for upper and lower bound of the driving corridor
    ...
    dist = []
    for i in pointsleft:
        dist.append(distance(vehpos,i))


    for i in range(len(dist)):
        if dist[i]==min(dist):
            limitleft = pointsleft[i]
            break

    dist2=[]
    for i in pointsright:
        dist2.append(distance(vehpos,i))


    for i in range(len(dist)):
        if dist[i]==min(dist):
            limitright= pointsright[i]
            break
